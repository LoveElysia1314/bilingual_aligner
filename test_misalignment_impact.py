...
def calculate_punctuation_stats_and_weights(en_lines, zh_lines):
    ...
    # 计算标点数和Correct alignment weights
...
def analyze_results(
    ...
    print(f"  Test sample pairs: {len(correct_weights)} 对")
    print()
    ...
    print(
        f"  绝对值Mean: {np.mean(np.abs(deviations)):.4f} ± {np.std(np.abs(deviations)):.4f}"
    )
    ...
    print(
        f"  绝对值Mean: {np.mean(np.abs(deviations_sqrt)):.4f} ± {np.std(np.abs(deviations_sqrt)):.4f}"
    )
    ...
    print(
        f"  绝对值Mean: {np.mean(np.abs(deviations_const)):.4f} ± {np.std(np.abs(deviations_const)):.4f}"
    )
    ...
    # Correct alignment weights统计
    correct_avg = np.mean(correct_weights)
    correct_std = np.std(correct_weights)
    print(f"Correct alignment weights基准: {correct_avg:.4f} ± {correct_std:.4f}")
    print(
